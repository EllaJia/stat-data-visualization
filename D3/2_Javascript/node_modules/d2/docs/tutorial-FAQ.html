...
<head>
    <meta charset="utf-8">
    <title>Tutorial: FAQ - Documentation</title>
...
<div id="main">
    ...
    <h1 class="page-title">Tutorial: FAQ</h1>
...
<article>
    <h1>How do i? (FAQ)</h1><h2>Figure out which models i can use?</h2><p>The types of models to use include almost all the available endpoint points in DHIS2. The ones available are the <em>generic</em> endpoints.
A more practical way to get the list is to use either the schemas endpoint, or use <code>d2</code> itself.</p>
<p>A request to the following url will get you all the names of schemas available. <code>http://play.dhis2.org/demo/api/schemas.json?fields=name</code></p>
<p>If you already have <code>d2</code> available you can do the following.</p>
<pre class="prettyprint source lang-js"><code>import { getInstance } from 'd2/lib/d2';

getInstance(d2 => {
    console.log(Object.keys(d2.models));
});</code></pre><h2>How do i add translation sources and strings and get rid of <code>** name **</code></h2><p>Before calling the <code>init</code> we might want to register some sources for translations. The translation sources could be either specified by adding a file source
(in the form of a <code>.properties</code> file) or by adding individual strings.</p>
<h3>Adding file sources</h3><p>To add a file source you simply add it to the set of files to load by using the <code>i18n.sources.add</code> method.</p>
<p>In a more real world example this would look something like the following.</p>
<pre class="prettyprint source lang-js"><code>const userLanguage = window.navigator.language.slice(0, 2);

if (userLanguage !== 'en') {
    config.i18n.sources.add(`./i18n/i18n_module_${userLanguage}.properties`);
}

config.i18n.sources.add('./i18n/i18n_module_en.properties');</code></pre><h3>Adding individual strings</h3><p>To add strings/keys that you would like to get translated by the dhis2 api (These are the translation keys that you do not specify in your <code>.properties</code> files)
you need to register them before calling the <code>init</code> method. Registration of the keys is done by caling <code>i18n.strings.add</code>.
The <code>init</code> method will then ask the <code>/api/i18n</code> endpoint for the translations of all the registered strings.</p>
<pre class="prettyprint source lang-js"><code>import { init, config } from 'd2/lib/d2';
...
init({ baseUrl: 'https://play.dhis2.org/demo/api' });</code></pre>
</article>
