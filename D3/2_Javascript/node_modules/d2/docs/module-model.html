...
<article>
    <div class="container-overview">
            ...
            <div class="description"><p>This page will give you an introduction on one of the largest and more complex parts of d2, the metadata models.</p>
<p>Models are an abstraction on top of the dhis2 metadata api. We can use these models to interact with the dhis2 api.
The models are accessible on the <code>.models</code> property that is attached to the d2 instance (that is retrieved by calling
<code>getInstance()</code>).</p>
<p>A schema is a description of a type of metadata object. To find out which schemas are available for you to use with
d2 take a look at https://play.dhis2.org/demo/api/schemas.json?fields=name,metadata. Any schema that is marked as
<code>metadata: true</code> can be used with d2.</p>
<p>There are a few concepts that you should familiarise yourself with before you start. (More documentation on the following
concepts can be found on the classes that represent them.)</p>
<h3>ModelDefinitions</h3><p>A <a href="module-model.ModelDefinition.html">module:model.ModelDefinition</a> is where it all starts. In d2 the schemas you have loaded are represented by a <code>ModelDefinition</code>.
A <code>ModelDefinition</code> is essentially a wrapper around the schemas that allows you to create <code>Model</code> instances for that
specific schema (e.g. the dataElementModelDefinition allows you to create dataElement Models).</p>
<pre class="prettyprint source lang-js"><code>d2.models.dataElement // This is the dataElement ModelDefinition
 .create()            // Creates a Model object based of the dataElement ModelDefinition</code></pre><p>When working with the d2 object the <code>d2.models</code> object contains modelDefinitions for the schemas that you have loaded.
(by default that is all schemas). To get a modelDefinition for a specific schema you ask for it by the name of the schema.
A few examples other than the dataElement one from above would be:</p>
<pre class="prettyprint source lang-js"><code>d2.models.user
d2.models.legendSet
d2.models.organisationUnit</code></pre><p>The ModelDefinition allows you to load objects from the api. You can either load individual items or items in bulk.</p>
<p>We can use it to get specific objects</p>
<pre class="prettyprint source lang-js"><code>d2.models.organisationUnit.get('ImspTQPwCqd')
 .then(organisationUnitModel => console.log(organisationUnitModel));</code></pre><p>or to load a list of objects (by default paging is enabled and it will only load the first 50 entries).</p>
<pre class="prettyprint source lang-js"><code>d2.models.organisationUnit.list()
 .then(organisationUnitCollection => console.log(organisationUnitCollection));</code></pre><p>The ModelDefinition also allows you to filter on properties using a programatic syntax. To find out which
filter methods are available you can a look at the <a href="module-model.Filter.html">Filter</a> class.</p>
<pre class="prettyprint source lang-js"><code>d2.models.organisationUnit
 .filter().on('level').equals(2)
 .list()
 .then(organisationUnitCollection => console.log(organisationUnitCollection));</code></pre><p>Applying a filter like this will create a clone of the modelDefinition with the filter applied, so you can
store the reference to the cloned modelDefinition and use it later, without globally applying the filter everywhere.</p>
<pre class="prettyprint source lang-js"><code>const organisationUnitsOnLevel3 = d2.models.organisationUnit
...
organisationUnitsOnLevel3WithParent
 .list({ paging: false }) // Loads all organisation units on level 3 with O6uvpzGd5pu as their parent
  .then(organisationUnitCollection => console.log(organisationUnitCollection));</code></pre><p>As you might have noticed we passed <code>{ paging: false }</code> to the <code>.list</code> method. Any options passed to the list method
(with the exception of filter) will be passed through as regular query parameters.</p>
<p>If you ran the above examples you would notice that the <code>.list</code> method returns a ModelCollection.</p>
<h2>ModelCollection</h2><p>A <a href="module-model.ModelCollection.html">ModelCollection</a> is simple a collection of Model objects. The ModelCollection is
sometimes somewhat cumbersome to work with and often you would like to transform the ModelCollection to an array using
the <code>.toArray()</code> method or spreading the values of the ModelCollection.</p>
<pre class="prettyprint source lang-js"><code>d2.models.organisationUnit
 .filter().on('level').equals(2)
 .list()
...
});</code></pre><p>The ModelCollection does not only contain the Models, it also gives you access to a <a href="module-model.Pager.html">Pager</a>.
The pager objects keeps track of which page was loaded and how many pages there are.</p>
<p>The following example will load the first page of organisation units and then keep loading a new page every second
until we run out of pages.</p>
<pre class="prettyprint source lang-js"><code>function loadNextPage(pager, callback) {
...
d2.models.organisationUnit
  ...
  });</code></pre><h2>Model</h2><p>TODO: :(</p></div>
...
<dl class="details">
    ...
    <dd class="tag-source"><ul class="dummy"><li>
        <a href="model_index.js.html">model/index.js</a>, <a href="model_index.js.html#line1">line 1</a>
    </li></ul></dd>
        ...
        <dl>
            <dt><a href="module-model.Filter.html">Filter</a></dt>
            <dd></dd>
        
            <dt><a href="module-model.Filters.html">Filters</a></dt>
            <dd></dd>
        
            <dt><a href="module-model.Model.html">Model</a></dt>
            <dd></dd>
        
            <dt><a href="module-model.ModelBase.html">ModelBase</a></dt>
            <dd></dd>
        
            <dt><a href="module-model.ModelCollection.html">ModelCollection</a></dt>
            <dd></dd>
        
            <dt><a href="module-model.ModelCollectionProperty.html">ModelCollectionProperty</a></dt>
            <dd></dd>
        
            <dt><a href="module-model.ModelDefinition.html">ModelDefinition</a></dt>
            <dd></dd>
        
            <dt><a href="module-model.ModelDefinitions.html">ModelDefinitions</a></dt>
            <dd></dd>
        
            <dt><a href="module-model.ModelValidation.html">ModelValidation</a></dt>
            <dd></dd>
        
            <dt><a href="module-model.Pager.html">Pager</a></dt>
            <dd></dd>
        </dl>
